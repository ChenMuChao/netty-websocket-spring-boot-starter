...
import com.adealink.weparty.annotation.EnableWebSocket;
import com.adealink.weparty.exception.DeploymentException;
import com.adealink.weparty.util.SpringContextUtils;
import com.adealink.weparty.annotation.ServerEndpoint;
...
import com.adealink.weparty.ws.pojo.PojoMethodMapping;
...
public class ServerEndpointExporter extends ApplicationObjectSupport implements SmartInitializingSingleton, BeanFactoryAware, ResourceLoaderAware {
    ...
    private void init() {
        boolean enableHttpServlet = SpringContextUtils.enableHttpServlet();
        for (Map.Entry<InetSocketAddress, WebsocketServer> entry : addressWebsocketServerMap.entrySet()) {
            WebsocketServer websocketServer = entry.getValue();
            try {
                websocketServer.init(enableHttpServlet);
                ...
                if (enableHttpServlet) {
                    logger.info(String.format("\033[34mNetty Http Server started on port: %s .\033[0m", pojoEndpointServer.getPort()));
                }
