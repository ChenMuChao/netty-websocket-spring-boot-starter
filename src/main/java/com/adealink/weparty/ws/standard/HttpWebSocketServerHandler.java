...
import com.adealink.weparty.ws.support.WsPathMatcher;
...
@Slf4j
class HttpWebSocketServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    private static ByteBuf buildStaticRes(String resPath) {
        try {
            InputStream inputStream = HttpWebSocketServerHandler.class.getResourceAsStream(resPath);
            if (inputStream != null) {
    ...
    public HttpWebSocketServerHandler(PojoEndpointServer pojoEndpointServer, ServerEndpointConfig config, EventExecutorGroup eventExecutorGroup, boolean isCors) {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest msg) throws Exception {
        try {
            ...
            log.error("handle http upgrade ws error", e);
        } catch (Exception e) {
    ...
    private void handleHttpRequest(ChannelHandlerContext ctx, FullHttpRequest req) {
        ...
        if (pojoEndpointServer.hasBeforeHandshake(channel, pattern)) {
            boolean canHandShake = pojoEndpointServer.doBeforeHandshake(channel, req, pattern);
            if (!canHandShake) {
                res = new DefaultFullHttpResponse(HTTP_1_1, NOT_ACCEPTABLE);
                sendHttpResponse(ctx, req, res);
                return;
            }
        ...
        if (handshaker == null) {
            ...
            handshaker.handshake(channel, req, responseHeader, channel.newPromise()).addListener(future -> {
                if (future.isSuccess()) {
                    if (isCors) {
