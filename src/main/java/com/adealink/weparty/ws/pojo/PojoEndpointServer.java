package com.adealink.weparty.ws.pojo;

import com.adealink.weparty.common.WebSocketAttrConstant;
import com.adealink.weparty.ws.standard.ServerEndpointConfig;
import com.adealink.weparty.ws.support.*;
...
public class PojoEndpointServer {
    ...
    private static final AttributeKey<Object> POJO_KEY = AttributeKey.valueOf(WebSocketAttrConstant.WEBSOCKET_IMPLEMENT);

    public static final AttributeKey<Session> SESSION_KEY = AttributeKey.valueOf(WebSocketAttrConstant.WEBSOCKET_SESSION);

    private static final AttributeKey<String> PATH_KEY = AttributeKey.valueOf(WebSocketAttrConstant.WEBSOCKET_PATH);

    public static final AttributeKey<Map<String, String>> URI_TEMPLATE = AttributeKey.valueOf(WebSocketAttrConstant.WEBSOCKET_URI_TEMPLATE);

    public static final AttributeKey<Map<String, List<String>>> REQUEST_PARAM = AttributeKey.valueOf(WebSocketAttrConstant.WEBSOCKET_REQUEST_PARAM);

    private final Map<String, PojoMethodMapping> pathMethodMappingMap = new HashMap<>();

    public static final AttributeKey<HttpHeaders> RESPONSE_HEADER_KEY = AttributeKey.valueOf(WebSocketAttrConstant.WEBSOCKET_RESPONSE_HEADER);
    ...
    public boolean doBeforeHandshake(Channel channel, FullHttpRequest req, String path) {
        ...
        HttpHeaders responseHeader = new DefaultHttpHeaders();
        channel.attr(RESPONSE_HEADER_KEY).set(responseHeader);
        Method beforeHandshake = methodMapping.getBeforeHandshake();
        if (beforeHandshake != null) {
            try {
                return (boolean) beforeHandshake.invoke(implement, methodMapping.getBeforeHandshakeArgs(channel, req));
